...
def plot_index_vs_ma_vwma(df_to_plot: pd.DataFrame, ps: PlotSetup) -> plt.Figure:
    """
    Plot index price and MA/VWMA overlays.
    Adds a 3rd y-axis per subplot for oscillators so they are visible.
    """
    df_to_plot = df_to_plot.loc[ps.price_data.index]
    ...
    # Third axis for oscillator (offset to the right)
    ax_osc = ax.twinx()
    ax_osc.spines["right"].set_position(("axes", 1.02))  # move it outward
    ax_osc.spines["right"].set_visible(True)

    ax.set_title(
        f"{ps.mkt} price vs MAs (with MA max/min ranges). {ps.sample_start}-{ps.sample_end}",
        fontsize=12, fontweight="bold"
    )

    # Price layer sets price ylim etc.
    ps.plot_price_layer(ax)

    # Plot MA lines on price axis
    for ma in core.constants.mas_list:
        col = f"MA{ma}"
        col_label = f"{ma}-day mov. avg"
        if col in df_to_plot.columns:
            ax.plot(
                ps.plot_index, df_to_plot[col],
                color=core.constants.ma_color_map.get(col, None),
                label=col_label, zorder=5
            )

    # Range bars on price axis (these are in price units)
    if 'MA_range' in df_to_plot.columns:
        ax.bar(ps.plot_index, df_to_plot['MA_range'], bottom=ps.ymin, width=1.0,
               label='Max/min MA range', alpha=0.8)
    if 'MA_no200_range' in df_to_plot.columns:
        ax.bar(ps.plot_index, df_to_plot['MA_no200_range'], bottom=ps.ymin, width=1.0,
               label='Max/min MA range (no 200day MA)', alpha=0.5)

    # Oscillators on osc axis (NOT price axis)
    if 'short_MA_osc' in df_to_plot.columns:
        ax_osc.bar(ps.plot_index, df_to_plot['short_MA_osc'],
                    color='lightgreen', linewidth=1.0, alpha=0.35, label='short_MA_osc')
    else:
        print("no short_MA_osc")

    if 'MA_no200_osc' in df_to_plot.columns:
        ax_osc.bar(ps.plot_index, df_to_plot['MA_no200_osc'],
                    color='green', linewidth=1.0, alpha=0.35, label='MA_no200_osc')
    else:
        print("no MA_no200_osc")

    # Choose osc limits (good defaults for z-score; change to (0,1) for minmax)
    ax_osc.set_ylim(-4, 4)
    ax_osc.axhline(0, color="black", linestyle="--", linewidth=1.0, alpha=0.6)
    ax_osc.set_ylabel("Oscillator", fontsize=9)
    ax_osc.grid(False)

    # Volume on vol axis (inverted)
    vol = df_to_plot['Volume'].astype(float)
    ax_vol.bar(ps.plot_index, -vol, width=1.0, alpha=0.3, label='Volume (inverted)')
    ax_vol.set_ylim(-vol.max(), 0)
    ax_vol.set_yticks([])
    ...
    lines3, labels3 = ax_osc.get_legend_handles_labels()
    ax_osc.legend(lines1 + lines2 + lines3, labels1 + labels2 + labels3,
    ...
    ax_osc = ax.twinx()
    ax_osc.spines["right"].set_position(("axes", 1.02))
    ax_osc.spines["right"].set_visible(True)

    ax.set_title(
        f"{ps.mkt} price vs VWMAs, with VWMA (max-min) ranges and range-oscillator",
        fontsize=12, fontweight="bold"
    )

    ps.plot_price_layer(ax)

    # VWMA lines on price axis
    for ma in core.constants.mas_list:
        col = f"VWMA{ma}"
    ...
    # Range bars on price axis
    if 'VWMA_range' in df_to_plot.columns:
        ax.bar(ps.plot_index, df_to_plot['VWMA_range'], bottom=ps.ymin, width=1.0,
               label='Max/min VWMA range', alpha=0.9)
    if 'VWMA_no200_range' in df_to_plot.columns:
        ax.bar(ps.plot_index, df_to_plot['VWMA_no200_range'], bottom=ps.ymin, width=1.0,
               label='Max/min VWMA range (no 200 day VWMA)', alpha=0.7)

    # Oscillators on osc axis
    if 'short_VWMA_osc' in df_to_plot.columns:
        ax_osc.bar(ps.plot_index, df_to_plot['short_VWMA_osc'],
                    color='lightgreen', linewidth=1.0, alpha=0.35, label='short_VWMA_osc')
    else:
        print("no short_VWMA_osc")

    if 'VWMA_no200_osc' in df_to_plot.columns:
        ax_osc.bar(ps.plot_index, df_to_plot['VWMA_no200_osc'],
                    color='green', linewidth=1.0, alpha=0.35, label='VWMA_no200_osc')
    else:
        print("no VWMA_no200_osc")
